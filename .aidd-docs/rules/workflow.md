...
- 開発用の資料（rules・flows・knowledge・project・vision・specs 等）はすべて隠しディレクトリ `.aidd-docs/` に置く。人間向けのファイルが誤って本体に混ざらないよう、開発機構は `.aidd-docs/` に集約する。プロジェクト固有の内容は `.aidd-docs/project/`（プロジェクト層）にのみ書き、テンプレート由来のファイルに書き込まない
