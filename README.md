...
|---|---|---|---|---|
| ① **ビジョン**（別枠・随時） | `/aidd-vision` | プロジェクトの大域方針を決める・見直すとき | `.aidd-docs/vision.md`（生きたドキュメント） | 対話で選択肢を決断し、ビジョン更新を承認 |
| ② **spec ループ**（開発の本流） | `/aidd-spec`→（`/aidd-design`）→`/aidd-tasks`→`/aidd-implement`→`/aidd-review` | 機能の追加・変更（仕様を書く価値がある規模） | 仕様（＋設計）＋タスク＋実装 = 1 PR | 仕様・（設計・）タスク・実装をレビューして承認 |
...
|---|---|---|---|
| `/aidd-vision <テーマ>` | ① | ビジョン（大域方針）を対話で固める・更新する | 人間と対話 |
| `/aidd-spec <要件>` | ② | 仕様書ドラフトを作成してレビュー依頼 | Claude |
| `/aidd-design [ループ名]` | ② | 承認済み仕様の実装構造を設計書に具体化してレビュー依頼（spec が `design: required` のループのみ） | Claude |
...
flowchart TD
    ...
    B --> C{人間: 仕様レビュー}
    C -- 修正指示 --> B
    C -- "承認（design: required）" --> DS["/aidd-design<br>Claude が設計書を作成"]
    DS --> DR{人間: 設計レビュー}
    DR -- 修正指示 --> DS
    DR -- 承認 --> D
    C -- "承認（design: skip）" --> D["/aidd-tasks<br>Claude が TDD 形式でタスク分解"]
    D --> E{人間: タスクレビュー}
    E -- 修正指示 --> D
...
```
/aidd-vision 全体アーキテクチャの方針を検討したい
```

spec ループとは**別枠**で、「このプロジェクトはこういう形を目指す」という大域方針＝**ビジョン**（`.aidd-docs/vision.md`）を対話で固めます。選択肢とトレードオフを1論点ずつ整理し、合意したら人間の承認を経てビジョンに反映します。

- ビジョンは**具体になりすぎない**抽象度（アーキテクチャの形・設計原則・優先順位・目指さない方向）を保ちます。対話で出た具体の決定は「**/aidd-spec への引き継ぎ事項**」として次のループに渡されます（経緯は `.aidd-docs/vision-notes/` の検討メモに残せます)
...
構造の検討が必要な規模の変更では、**設計フェーズの要否**（`design: required` / `skip`）も理由付きで提案されます（確定するのはあなたです）。
仕様を読んで、修正指示を出すか「**承認**」と返信してください。

### 2. （条件付き）設計を具体化させる

```
/aidd-design 001-user-auth
```

仕様が `design: required`（新モジュール・新しいデータ構造・外部 API の新設など）の場合だけ挟まるフェーズです。仕様（振る舞い）を実現する**構造** — モジュール構成・データ構造・インターフェース・主要フロー — が `.aidd-docs/specs/001-user-auth/design.md` にまとまり、レビューを依頼してきます。確認して修正指示か「**承認**」。

- 仕様（what）と設計（how）を分けるのは**変更頻度が違う**ためです。実装中に構造を変えたくなっても、振る舞いが変わらなければ design.md の修正＋再承認だけで済み、承認済みの仕様を開き直しません
- 小さい変更（`design: skip`）ではこのフェーズはなく、仕様の「§5 技術方針」が設計の置き場になります
...
自律実装はタスクを上から順に「実装中マーク → テスト作成/実装 → 検証 → チェック」のサイクルで進めます。コミットは細かく刻まず、**フェーズ0完了時と実装完了時の2回**にまとまります（承認プロンプトを増やさないため）。レビュー中（`status: draft`）の spec.md / design.md / tasks.md はコミットされず、承認後にこの2回のコミットへ含まれます。
...
- `spec.md` / `design.md` / `tasks.md` の frontmatter `status`（draft → approved → in-progress → done）が状態のソースオブトゥルースです。
...
- 承認後は**既定で次のフェーズに自動継続**します（spec承認→設計またはタスク分解へ、design承認→タスク分解へ、tasks承認→実装へ）。自動継続した先も必ず自身のレビュー依頼で停止するため、承認ゲートは維持されます。継続してほしくないときは `/aidd-approve <ループ名> stop`。
...
| `spec-writing` | 機能要件・非機能要件のチェックリスト、仕様アンチパターン | /aidd-vision /aidd-spec |
| `tdd-antipatterns` | テストケース洗い出し手法（境界値・デシジョンテーブル等）、TDDアンチパターン集 | /aidd-tasks /aidd-implement |
| `security-practices` | 項目別の「ベスト / 一時回避（返済条件付き）/ 禁止」3段階ガイド | /aidd-spec /aidd-design /aidd-implement /aidd-review |
| `project-structure` | 結合度を下げるディレクトリ構成の原則・判断テスト | /aidd-spec /aidd-design /aidd-implement |
| `infra-architecture` | コンテナ・ネットワーク構成のベストプラクティス | /aidd-vision /aidd-design /aidd-spec /aidd-implement |
...
**テンプレートに入れない知識**: 言語別コーディング規約（PEP8等）やデザイン規約は種類が多く時流で変わるため含めていません。プロジェクト層の `.aidd-docs/project/context.md` に書くか、`.aidd-docs/project/knowledge/` に知識ファイルとして追加してください（テンプレート側の `knowledge/` には追加しない。後述の「テンプレートの更新を取り込む」参照）。

## ディレクトリ構成

**本体（エージェント中立）はすべて `.aidd-docs/`** にあり、エージェント固有ディレクトリ（`.claude/` 等）には本体を指す薄いアダプタしか置きません。開発用の資料一式を隠しディレクトリ `.aidd-docs/` にまとめることで、人間向けのファイルが誤って本体に混ざるのを防ぎます。さらに、**プロジェクト固有の内容はプロジェクト層（`.aidd-docs/project/` 等。★印）に分離**されており、テンプレート本体には書き込みません（詳細は「テンプレートの更新を取り込む」）。**人間向けの現在仕様ドキュメント（`docs/current-spec.md`）だけは `docs/` に生成されます** — `docs/` はプロジェクトのユーザー向けドキュメント領域なので、その他のユーザー向け資料と同じ場所で参照できます。

```
AGENTS.md                  # エージェント共通エントリポイント（絶対ルール＋操作→手順書の対応表）
CLAUDE.md                  # Claude Code 用アダプタ（AGENTS.md・project/context.md・rules/ を @import）
...
├── flows/                 # 全11操作の手順書本体（README.md に表記規約）
├── knowledge/             # 知識ファイル10種（各手順書が冒頭で読み込む）
├── project/               # ★プロジェクト層: 固有内容の唯一の置き場（テンプレート更新で上書きされない）
│   ├── context.md         # プロジェクト概要・スタック・コマンド・固有ルール（CLAUDE.md が @import）
│   └── knowledge/         # プロジェクト固有の知識ファイル（読むタイミングを context.md の表に登録）
├── vision.md              # ★ビジョン: 目指す形の大域合意＋実現の記録（/aidd-vision で更新）
├── vision-notes/          # ★/aidd-vision の検討メモ（経緯と /aidd-spec への引き継ぎ事項。履歴）
└── specs/
    ├── _template/         # spec.md / design.md / tasks.md のテンプレート
    └── NNN-機能名/        # ★ループごとの仕様・設計・タスク（Claude が作成、履歴として蓄積）
docs/                      # ★ユーザー向けドキュメント領域
└── current-spec.md        # 人間向けの現在仕様ドキュメント（/aidd-docs がコードとテストから再生成する生成物）
```

★ = プロジェクト層（プロジェクトが所有。テンプレート更新で触らない）。無印はテンプレート層。
...
## テンプレートの更新を取り込む（テンプレート層とプロジェクト層）

プロジェクト固有の内容とテンプレート本体は、依存性の注入（DI）と同じ発想で分離されています。テンプレート層は固有内容を**参照はするが含まず**（CLAUDE.md / AGENTS.md が `.aidd-docs/project/context.md` を注入ポイントとして読み込む）、**固有の内容は必ずプロジェクト層に書きます**。トークン効率のためプロジェクト層は2段構えです — `context.md` は毎セッション読み込まれる**薄い索引**（概要・コマンド・常時ルール・知識の登録表。目安50行）、分量のある固有知識は `project/knowledge/` に置かれ**該当する作業のときだけ**読み込まれます:

| 層 | パス | テンプレート更新時 |
|---|---|---|
| **テンプレート層**（仕組み） | `.aidd-docs/` の `rules/` `flows/` `knowledge/` `specs/_template/`、`AGENTS.md`、`CLAUDE.md`、`.claude/commands/`・`.claude/skills/` のテンプレート由来分 | **丸ごと上書きして取り込む** |
| **プロジェクト層**（固有内容） | `.aidd-docs/project/`、`.aidd-docs/vision.md`・`vision-notes/`・`specs/NNN-*`、`docs/`、`README.md`、`.claude/settings.json` | 触らない（プロジェクトが所有） |

- **更新手順**: テンプレート層のパスを新しいテンプレートの内容で上書きし、`git diff` を確認してコミットするだけです。プロジェクト層は差分に出ないはずで、出た場合は固有内容がテンプレート層に混入しているサインです（`.aidd-docs/project/` へ移してください）
- `.claude/settings.json` は初期値だけテンプレートが提供します。プロジェクトで許可リストを育てている場合、テンプレート側の変更は手動でマージしてください
- プロジェクト固有のスラッシュコマンド・スキルの追加は自由です（上書き対象はテンプレート由来の名前だけなので更新で消えません）。ただし本文は `.aidd-docs/project/knowledge/` に置き、`.claude/` 側は薄いローダーにする規約はテンプレートと同じです

## このテンプレートを使い始めるには

1. このディレクトリ一式を新しいプロジェクトにコピー（または `git init` してベースにする）
2. `.aidd-docs/project/context.md` にプロジェクト概要・スタック・テスト/ビルドコマンドを書く（プロジェクト固有の内容は以後もここ＝プロジェクト層にだけ書く）
